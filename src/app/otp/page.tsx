...
export default function OTPPage() {
  ...
  const handleInput = (i: number, val: string) => {
    ...
    if (d.every(x => x) && d.join('').length === 6) setTimeout(() => verify(d.join('')), 150)
  }
  ...
  const resendOTP = async () => {
    ...
    setResend(42)
    showPopup({ type: 'info', title: 'OTP Resent', body: `A new code has been sent to +91 ${phone}`, icon: '📱', actions: [{ label: 'OK', variant: 'primary', fn: closePopup }] })
  }
    ...
    <div style={{ minHeight: '100vh', background: '#0a0f1e', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
      <div style={{ width: '390px', height: '844px', borderRadius: '48px', overflow: 'hidden', position: 'relative', flexShrink: 0, boxShadow: '0 0 0 10px #1e293b, 0 0 0 12px #334155, 0 40px 80px rgba(0,0,0,0.8)', background: '#fff' }}>
        ...
        <div style={{ position: 'absolute', inset: 0, display: 'flex', flexDirection: 'column', background: '#fff' }}>
          ...
          {/* Status bar */}
          <div style={{ height: '50px', padding: '14px 20px 0', display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexShrink: 0 }}>
          ...
          <div style={{ height: '56px', background: '#fff', display: 'flex', alignItems: 'center', padding: '0 16px', gap: '10px', borderBottom: '1px solid #E2E8F0', flexShrink: 0 }}>
            ...
            <span style={{ flex: 1, textAlign: 'center', fontSize: '17px', fontWeight: 800, color: '#0F172A', letterSpacing: '-0.3px' }}>Verify Number</span>
          ...
          <div style={{ flex: 1, overflowY: 'auto', padding: '20px 28px' }}>
            ...
            {/* Icon */}
            <div style={{ width: '56px', height: '56px', borderRadius: '18px', background: '#FEF3C7', display: 'flex', alignItems: 'center', justifyContent: 'center', marginBottom: '20px' }}>
              <svg width="28" height="28" viewBox="0 0 24 24" fill="none">
                <rect x="2" y="4" width="20" height="16" rx="3" stroke="#D97706" strokeWidth="1.5"/>
                <path d="M2 8l10 6 10-6" stroke="#D97706" strokeWidth="1.5" strokeLinecap="round"/>
              </svg>
            </div>

            <h1 style={{ fontSize: '28px', fontWeight: 900, color: '#0F172A', letterSpacing: '-0.6px', margin: 0 }}>Check your SMS</h1>
            <p style={{ fontSize: '15px', color: '#64748B', marginTop: '8px', lineHeight: 1.55 }}>
              6-digit code sent to <strong style={{ color: '#0F172A' }}>+91 {phone}</strong>
            </p>

            {/* OTP Boxes */}
            <div style={{ display: 'flex', gap: '10px', justifyContent: 'center', margin: '28px 0 12px' }}>
              {digits.map((d, i) => (
                <input key={i} ref={el => { refs.current[i] = el }} value={d} maxLength={1} inputMode="numeric"
                  onChange={e => handleInput(i, e.target.value)}
                  onKeyDown={e => handleBackspace(i, e.key)}
                  style={{ width: '46px', height: '58px', borderRadius: '12px', textAlign: 'center', fontSize: '24px', fontWeight: 800, fontFamily: 'monospace', border: `2px solid ${d ? '#0D9488' : '#E2E8F0'}`, background: d ? '#EDFAF7' : '#F8FAFC', color: d ? '#0D9488' : '#0F172A', outline: 'none', transition: 'all 0.2s', boxSizing: 'border-box' }}
                  onFocus={e => { if (!e.target.value) { e.target.style.borderColor = '#0D9488'; e.target.style.background = '#fff' } }}
                />
              ))}
            </div>
            ...
            <p style={{ textAlign: 'center', fontSize: '13px', color: '#94A3B8', marginBottom: '24px' }}>
              {resend > 0
                ...
                : <span onClick={resendOTP} style={{ color: '#0D9488', fontWeight: 700, cursor: 'pointer' }}>Resend OTP</span>
              }
            </p>
                  ...
                  <button key={idx}
                    onClick={() => {
                      if (k === '⌫') {
                        ...
                        if (last >= 0) { const d = [...digits]; d[last] = ''; setDigits(d); refs.current[last]?.focus() }
                      } else {
                        const first = digits.findIndex(d => !d)
                        if (first >= 0) handleInput(first, k)
                      }
                    }}
                    style={{ padding: '16px', background: '#F1F5F9', border: '1px solid #E2E8F0', borderRadius: '14px', fontSize: '20px', fontWeight: 700, cursor: 'pointer', fontFamily: 'DM Sans, sans-serif', transition: 'background 0.15s' }}
            ...
            <button onClick={() => verify()} disabled={loading}
              style={{ width: '100%', padding: '16px', background: loading ? '#94A3B8' : '#0D9488', border: 'none', borderRadius: '14px', color: '#fff', fontSize: '16px', fontWeight: 700, cursor: loading ? 'not-allowed' : 'pointer', fontFamily: 'DM Sans, sans-serif' }}>
              {loading ? 'Verifying…' : 'Verify & Continue →'}
            </button>

            <div style={{ textAlign: 'center', marginTop: '16px' }}>
              <span onClick={() => router.back()} style={{ fontSize: '13px', color: '#64748B', fontWeight: 600, cursor: 'pointer' }}>
                ← Change number
              </span>
            </div>
          </div>
