...
function ContactSheet({ open, onClose, onSave, initial }: { open:boolean; onClose:()=>void; onSave:(c:Omit<EmergencyContact,'id'>)=>void; initial?:EmergencyContact|null }) {
  const [name,setName]=useState(initial?.name||''); const [phone,setPhone]=useState(initial?.phone||''); const [relation,setRelation]=useState(initial?.relation||''); const [primary,setPrimary]=useState(initial?.isPrimary||false)
  const relations=['Spouse','Parent','Sibling','Child','Friend','Colleague','Other']
  const save=()=>{ if(!name.trim()||!phone.trim()||!relation.trim()) return; onSave({name:name.trim(),phone:phone.trim(),relation,isPrimary:primary}); onClose() }
  if(!open) return null
  const inp:React.CSSProperties={ width:'100%', padding:'12px 14px', background:'#F8FAFC', border:'1.5px solid #E2E8F0', borderRadius:'12px', fontSize:'14px', color:'#0F172A', fontFamily:'DM Sans, sans-serif', outline:'none', boxSizing:'border-box' }
  return (
    <div onClick={e=>{ if(e.target===e.currentTarget) onClose() }} style={{ position:'absolute', inset:0, zIndex:200, background:'rgba(15,23,42,0.55)', display:'flex', flexDirection:'column', justifyContent:'flex-end' }}>
      <div style={{ background:'#fff', borderRadius:'28px 28px 0 0', padding:'0 20px 36px', maxHeight:'88%', overflowY:'auto' }}>
        ...
        <h3 style={{ fontSize:'20px', fontWeight:800, color:'#0F172A', letterSpacing:'-0.4px', marginBottom:'18px' }}>{initial?'Edit Contact':'Add Emergency Contact'}</h3>
        ...
        <div style={{ display:'flex', flexWrap:'wrap', gap:'8px', marginBottom:'16px' }}>
          {relations.map(r => <button key={r} onClick={()=>setRelation(r)} style={{ padding:'7px 16px', borderRadius:'100px', border:'none', cursor:'pointer', fontSize:'13px', fontWeight:700, fontFamily:'DM Sans, sans-serif', background:relation===r?'#0D9488':'#F1F5F9', color:relation===r?'#fff':'#475569', transition:'all 0.15s' }}>{r}</button>)}
        </div>
        <div onClick={()=>setPrimary(v=>!v)} style={{ display:'flex', alignItems:'center', justifyContent:'space-between', background:'#F8FAFC', border:'1.5px solid #E2E8F0', borderRadius:'14px', padding:'13px 16px', marginBottom:'20px', cursor:'pointer' }}>
          <div><div style={{ fontSize:'14px', fontWeight:700, color:'#0F172A' }}>Set as Primary Contact</div><div style={{ fontSize:'12px', color:'#64748B', marginTop:'2px' }}>Called first in an emergency</div></div>
        ...
        <button onClick={save} disabled={!name||!phone||!relation} style={{ width:'100%', padding:'16px', background:(!name||!phone||!relation)?'#CBD5E1':'#0D9488', border:'none', borderRadius:'14px', color:'#fff', fontSize:'16px', fontWeight:700, cursor:(!name||!phone||!relation)?'not-allowed':'pointer', fontFamily:'DM Sans, sans-serif', marginBottom:'10px' }}>{initial?'Save Changes':'Add Contact'} →</button>
...
function ContactCard({ contact,index,onEdit,onDelete,onCall }: { contact:EmergencyContact;index:number;onEdit:()=>void;onDelete:()=>void;onCall:()=>void }) {
  ...
  return (
    <div style={{ margin:'0 14px 10px', background:'#fff', borderRadius:'18px', padding:'14px 16px', border:`1.5px solid ${contact.isPrimary?'#CCFBF1':'#E2E8F0'}`, position:'relative' }}>
      {contact.isPrimary&&<div style={{ position:'absolute', top:'10px', right:'14px', background:'#EDFAF7', color:'#0D9488', fontSize:'10px', fontWeight:700, padding:'3px 9px', borderRadius:'100px', letterSpacing:'0.3px' }}>★ Primary</div>}
      <div style={{ display:'flex', alignItems:'center', gap:'13px', marginBottom:'12px' }}>
        <div style={{ width:'48px', height:'48px', borderRadius:'50%', background:color, display:'flex', alignItems:'center', justifyContent:'center', fontSize:'16px', fontWeight:800, color:'#fff', flexShrink:0 }}>{initials}</div>
      ...
      <div style={{ display:'flex', gap:'8px' }}>
        <button onClick={onCall} style={{ flex:1, padding:'10px', background:'#EDFAF7', border:'1px solid #CCFBF1', borderRadius:'12px', fontSize:'13px', fontWeight:700, color:'#0D9488', cursor:'pointer', fontFamily:'DM Sans, sans-serif', display:'flex', alignItems:'center', justifyContent:'center', gap:'5px' }}>
          <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="#0D9488" strokeWidth="2.5" strokeLinecap="round"><path d="M22 16.92v3a2 2 0 01-2.18 2 19.79 19.79 0 01-8.63-3.07 19.5 19.5 0 01-6-6A19.79 19.79 0 012.12 8.7 2 2 0 014.11 6.5h3a2 2 0 012 1.72"/></svg>Call
        </button>
        <button onClick={onEdit} style={{ flex:1, padding:'10px', background:'#F8FAFC', border:'1px solid #E2E8F0', borderRadius:'12px', fontSize:'13px', fontWeight:700, color:'#475569', cursor:'pointer', fontFamily:'DM Sans, sans-serif', display:'flex', alignItems:'center', justifyContent:'center', gap:'5px' }}>
          <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="#475569" strokeWidth="2.5" strokeLinecap="round"><path d="M11 4H4a2 2 0 00-2 2v14a2 2 0 002 2h14a2 2 0 002-2v-7"/><path d="M18.5 2.5a2.121 2.121 0 013 3L12 15l-4 1 1-4 9.5-9.5z"/></svg>Edit
        </button>
...
export default function EmergencyPage() {
  ...
  const contacts=emergencyContacts; const setContacts=(c:EmergencyContact[])=>setEmergencyContacts(c)
  const [sheetOpen,setSheetOpen]=useState(false); const [editContact,setEditContact]=useState<EmergencyContact|null>(null)
  ...
  const saveContact=(data:Omit<EmergencyContact,'id'>)=>{
    if(editContact){ const updated=contacts.map(c=>c.id===editContact.id?{...c,...data}:c); const final=data.isPrimary?updated.map(c=>({...c,isPrimary:c.id===editContact.id})):updated; setContacts(final); showPopup({type:'success',title:'Contact Updated ✅',body:`${data.name}'s details have been saved.`,icon:'✅',actions:[{label:'OK',variant:'primary',fn:closePopup}]}) }
    else { const newC:EmergencyContact={...data,id:`ec${Date.now()}`}; const updated=data.isPrimary?[...contacts.map(c=>({...c,isPrimary:false})),newC]:[...contacts,newC]; setContacts(updated); showPopup({type:'success',title:'Contact Added ✅',body:`${data.name} has been added as an emergency contact.`,icon:'✅',actions:[{label:'OK',variant:'primary',fn:closePopup}]}) }
  }

  const deleteContact=(c:EmergencyContact)=>showPopup({ type:'confirm', title:'Remove Contact?', body:`Remove ${c.name} from your emergency contacts?`, icon:'🗑️', actions:[{ label:'Remove', variant:'danger', fn:()=>{ setContacts(contacts.filter(x=>x.id!==c.id)); closePopup() } },{ label:'Cancel', variant:'secondary', fn:closePopup }] })
  const callContact=(c:EmergencyContact)=>showPopup({ type:'info', title:`Call ${c.name}?`, body:`${c.relation} · ${c.phone}`, icon:'📞', actions:[{ label:'📞 Call Now', variant:'primary', fn:()=>{ closePopup(); showPopup({type:'success',title:'Calling…',body:`Dialling ${c.phone}`,icon:'📞',actions:[{label:'End Call',variant:'danger',fn:closePopup}]}) } },{ label:'Cancel', variant:'secondary', fn:closePopup }] })
  ...
  return (
    <MobileFrame>
      <div style={{ position:'absolute', inset:0, display:'flex', flexDirection:'column' }}>
        ...
        <div style={{ flex:1, overflowY:'auto', paddingBottom:'24px' }}>
          <div style={{ margin:'14px 14px 16px', background:'#FEF3C7', borderRadius:'16px', padding:'13px 16px', display:'flex', gap:'10px', border:'1px solid #FDE68A' }}>
            ...
            <div style={{ fontSize:'13px', color:'#92400E', lineHeight:1.55 }}>These contacts will be <strong>notified immediately</strong> when you trigger an SOS alert, along with your live location.</div>
          </div>
          <div style={{ fontSize:'11px', fontWeight:700, color:'#94A3B8', textTransform:'uppercase', letterSpacing:'0.6px', padding:'0 16px 10px' }}>{contacts.length} Contact{contacts.length!==1?'s':''} · Max 5</div>
          {contacts.length===0&&(
            <div style={{ display:'flex', flexDirection:'column', alignItems:'center', justifyContent:'center', padding:'40px 24px', gap:'12px', textAlign:'center' }}>
              ...
              <div style={{ fontSize:'13px', color:'#94A3B8', lineHeight:1.6 }}>Add up to 5 emergency contacts who will be alerted when you trigger SOS.</div>
              <button onClick={openAdd} style={{ marginTop:'8px', padding:'12px 28px', background:'#0D9488', border:'none', borderRadius:'14px', color:'#fff', fontSize:'14px', fontWeight:700, cursor:'pointer', fontFamily:'DM Sans, sans-serif' }}>+ Add First Contact</button>
            </div>
          )}
          {contacts.map((c,i)=><ContactCard key={c.id} contact={c} index={i} onEdit={()=>openEdit(c)} onDelete={()=>deleteContact(c)} onCall={()=>callContact(c)} />)}
          {contacts.length>0&&contacts.length<5&&(
            <div style={{ margin:'4px 14px 0' }}>
              <button onClick={openAdd} style={{ width:'100%', padding:'14px', background:'transparent', border:'2px dashed #CBD5E1', borderRadius:'18px', fontSize:'14px', fontWeight:700, color:'#64748B', cursor:'pointer', fontFamily:'DM Sans, sans-serif', display:'flex', alignItems:'center', justifyContent:'center', gap:'8px' }}>
                <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="#94A3B8" strokeWidth="2.5" strokeLinecap="round"><line x1="12" y1="5" x2="12" y2="19"/><line x1="5" y1="12" x2="19" y2="12"/></svg>Add Emergency Contact
              </button>
            </div>
          )}
      ...
      <PopupLayer />
      <ContactSheet open={sheetOpen} onClose={()=>setSheetOpen(false)} onSave={saveContact} initial={editContact} />
    </MobileFrame>
  )
}