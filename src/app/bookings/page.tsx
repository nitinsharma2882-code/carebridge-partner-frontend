...
function BookingCard({ booking, onPress }: { booking:Booking; onPress:()=>void }) {
  const SC = {
    upcoming:  { bg:'#EDFAF7', color:'#0D9488', label:'Upcoming' },
        ...
        <div style={{ background:'#fff', padding:'10px 14px', display:'flex', gap:'8px', flexShrink:0, borderBottom:'1px solid #E2E8F0' }}>
          {TABS.map(t => (
            <button key={t.key} onClick={() => setActiveTab(t.key)}
              style={{ flex:1, padding:'8px 4px', borderRadius:'12px', border:'none', cursor:'pointer', background:activeTab===t.key?'#0D9488':'#F1F5F9', fontFamily:'DM Sans, sans-serif', transition:'all 0.2s' }}>
        ...
        <div style={{ flex:1, overflowY:'auto', padding:'12px 0 24px' }}>
          {filtered.length===0 ? (
            <div style={{ display:'flex', flexDirection:'column', alignItems:'center', justifyContent:'center', padding:'60px 24px', gap:'12px', textAlign:'center' }}>
              <div style={{ fontSize:'52px' }}>{EMPTY[activeTab].icon}</div>
              <div style={{ fontSize:'16px', fontWeight:800, color:'#0F172A' }}>{EMPTY[activeTab].title}</div>
              <div style={{ fontSize:'13px', color:'#94A3B8', lineHeight:1.6 }}>{EMPTY[activeTab].sub}</div>
            </div>
          ) : filtered.map(b => (
