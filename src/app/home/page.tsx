...
export default function HomePage() {
  ...
  const onMouseMove = (e: React.MouseEvent) => {
    ...
    setFabPos({ x:Math.max(27,Math.min(r.width-27,e.clientX-r.left)), y:Math.max(27,Math.min(r.height-27,e.clientY-r.top)) })
  }
  ...
  const onTouchMove  = (e: React.TouchEvent) => {
    ...
    const r = frameRef.current?.getBoundingClientRect(); if (!r) return
    const t = e.touches[0]
    setFabPos({ x:Math.max(27,Math.min(r.width-27,t.clientX-r.left)), y:Math.max(27,Math.min(r.height-27,t.clientY-r.top)) })
    e.preventDefault()
  }
  const onTouchEnd = () => { dragging.current=false }

  const toggleOnline = () => {
    const next = !isOnline; setIsOnline(next); setOnlineTime(next ? 'Online for 0h 0m' : 'Go online to earn')
    showPopup({ type:next?'success':'info', title:next?'You are Online 🟢':'You are Offline ⚫', body:next?'You are now visible to customers.\nRequests will appear here.':"You won't receive any requests while offline.", icon:next?'🟢':'⚫', actions:[{ label:'OK', variant:'primary', fn:closePopup }] })
  }

  const handleSOS = () => { if (didDrag.current) return; setSosVisible(true) }

  const handleDocUpload = () => {
    showPopup({ type:'info', title:'Upload Medical Document', body:'Upload your medical documents securely.\nSupported: PDF, JPG, PNG (max 10MB)', icon:'📤',
      actions:[
  ...
  const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file.size > 10*1024*1024) { showPopup({ type:'error', title:'File Too Large', body:'Please select a file under 10MB.', icon:'❌', actions:[{ label:'OK', variant:'primary', fn:closePopup }] }); return }
    showPopup({ type:'success', title:'Uploaded! ✅', body:`${file.name} uploaded successfully.`, icon:'✅', actions:[{ label:'Done', variant:'primary', fn:closePopup }, { label:'View Docs', variant:'secondary', fn:() => { closePopup(); router.push('/documents') } }] })
    e.target.value=''
  }
        ...
        <div style={{ position:'absolute', inset:0, display:'flex', flexDirection:'column' }}>
          ...
          <div style={{ background:'#fff', padding:'10px 16px 14px', display:'flex', alignItems:'center', justifyContent:'space-between', borderBottom:'1px solid #E2E8F0', flexShrink:0 }}>
            ...
            <div style={{ display:'flex', alignItems:'center', gap:'8px' }}>
              <button onClick={handleSOS} style={{ display:'flex', alignItems:'center', gap:'5px', padding:'7px 11px', background:'#DC2626', border:'none', borderRadius:'20px', cursor:'pointer', animation:'sosPulse 2s infinite' }}>
              ...
              <button onClick={() => router.push('/notifications')} style={{ width:'38px', height:'38px', borderRadius:'50%', background:'#F1F5F9', border:'none', display:'flex', alignItems:'center', justifyContent:'center', cursor:'pointer', position:'relative' }}>
          ...
          <div style={{ background:'#0F172A', padding:'7px 0', flexShrink:0, overflow:'hidden' }}>
            <div style={{ display:'flex', gap:'0', animation:'marquee 22s linear infinite', whiteSpace:'nowrap' }}>
              {[...BULLETIN_ITEMS, ...BULLETIN_ITEMS].map((item, i) => (
                <span key={i} style={{ fontSize:'11px', fontWeight:600, color:'#94A3B8', paddingRight:'32px', flexShrink:0 }}>
                  {item}
                </span>
              ))}
            </div>
          ...
          <button onMouseDown={onMouseDown} onTouchStart={onTouchStart} onTouchMove={onTouchMove} onTouchEnd={onTouchEnd} onClick={handleSOS}
            style={{ position:'absolute', left:`${fabPos.x-27}px`, top:`${fabPos.y-27}px`, zIndex:40, width:'54px', height:'54px', borderRadius:'50%', background:'#DC2626', border:'none', cursor:'grab', display:'flex', flexDirection:'column', alignItems:'center', justifyContent:'center', boxShadow:'0 4px 16px rgba(220,38,38,0.5)', animation:'sosPulse 2s infinite', userSelect:'none', touchAction:'none' }}>
