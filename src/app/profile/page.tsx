...
function Field({ label, value, onChange, placeholder, readOnly=false, type='text' }: { label:string; value:string; onChange?:(v:string)=>void; placeholder?:string; readOnly?:boolean; type?:string }) {
  return (
...
function MenuItem({ icon, label, sub, danger=false, onClick }: { icon:string; label:string; sub?:string; danger?:boolean; onClick:()=>void }) {
  return (
  ...
  return (
    <div style={{ position:'absolute', inset:0, display:'flex', flexDirection:'column', background:'#F8FAFC' }}>
      ...
      <div style={{ flex:1, overflowY:'auto', padding:'16px' }}>
        ...
        {error && <div style={{ background:'#FEE2E2', border:'1px solid #FECACA', borderRadius:'12px', padding:'12px 16px', marginBottom:'14px', fontSize:'13px', color:'#DC2626', fontWeight:600 }}>⚠️ {error}</div>}
        ...
        <div style={{ marginBottom:'14px' }}>
          ...
          <div style={{ display:'flex', gap:'8px' }}>
            <div style={{ padding:'14px 12px', border:'1.5px solid #E2E8F0', borderRadius:'14px', fontSize:'14px', color:'#94A3B8', background:'#F8FAFC', fontWeight:700, flexShrink:0 }}>+91</div>
            <input type="tel" value={phone} onChange={e=>setPhone(e.target.value.replace(/\D/g,'').slice(0,10))} placeholder="Enter 10-digit number"
              style={{ flex:1, padding:'14px 16px', border:'1.5px solid #0D9488', borderRadius:'14px', fontSize:'14px', color:'#0F172A', background:'#fff', outline:'none', fontFamily:'DM Sans, sans-serif', boxSizing:'border-box' as const }} />
          </div>
        </div>
        ...
        <div style={{ marginBottom:'14px' }}>
          ...
          <input type="number" value={age} placeholder="Enter your age" onChange={e=>setAge(e.target.value)}
            style={{ width:'100%', padding:'14px 16px', border:'1.5px solid #0D9488', borderRadius:'14px', fontSize:'14px', color:'#0F172A', background:'#fff', outline:'none', fontFamily:'DM Sans, sans-serif', boxSizing:'border-box', MozAppearance:'textfield' } as React.CSSProperties} />
...
export default function ProfilePage() {
  ...
  const displayPhone = currentUser?.phone
    ? `+91 ${currentUser.phone.replace(/^\+?91/,'')}`
    : ''
    ...
    <MobileFrame>
      <div style={{ position:'absolute', inset:0, display:'flex', flexDirection:'column' }}>
        ...
        <div style={{ height:'56px', background:'#fff', display:'flex', alignItems:'center', padding:'0 16px', gap:'10px', borderBottom:'1px solid #E2E8F0', flexShrink:0 }}>
          ...
          <button onClick={()=>setView('personal')} style={{ background:'#EDFAF7', border:'none', borderRadius:'10px', padding:'0 14px', height:'34px', fontSize:'13px', fontWeight:700, color:'#0D9488', cursor:'pointer', fontFamily:'DM Sans, sans-serif' }}>Edit</button>
        </div>
        ...
        <div style={{ flex:1, overflowY:'auto', paddingBottom:'90px' }}>
          ...
          <div style={{ background:'linear-gradient(160deg,#065f52,#0D9488,#14b8a6)', padding:'24px 20px 28px', display:'flex', flexDirection:'column', alignItems:'center', gap:'10px' }}>
            ...
            <div style={{ textAlign:'center' }}>
              <div style={{ fontSize:'22px', fontWeight:900, color:'#fff', letterSpacing:'-0.4px' }}>{currentUser?.name||'Loading…'}</div>
              <div style={{ fontSize:'13px', color:'rgba(255,255,255,0.75)', marginTop:'3px' }}>Healthcare Assistant · {displayPhone}</div>
            </div>
