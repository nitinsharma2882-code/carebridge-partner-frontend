...
export default function ProfilePage() {
  ...
  const saveProfile = () => {
    setEditMode(false)
    showPopup({
      type: 'success', title: 'Profile Updated ✅',
  ...
  const onDocPress = (doc: typeof DOCS[0]) => {
    ...
    showPopup({
      ...
      body: doc.status === 'verified'
        ? 'This document has been verified by CareBridge.'
        : doc.status === 'pending'
        ? 'Document is under review. Allow 24–48 hours.'
        : doc.status === 'rejected'
        ? 'Document was rejected. Please re-upload a clearer copy.'
  ...
  const inputStyle = (editable: boolean): React.CSSProperties => ({
    ...
    border: `1.5px solid ${editable ? '#0D9488' : '#E2E8F0'}`,
          ...
          <div style={{ flex: 1, overflowY: 'auto', paddingBottom: '24px' }}>
            ...
            <div style={{ background: 'linear-gradient(160deg,#065f52,#0D9488,#14b8a6)', padding: '24px 20px 28px', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '10px' }}>
              ...
              <div style={{ position: 'relative' }}>
                ...
                {editMode && (
                  <div style={{ position: 'absolute', bottom: 0, right: 0, width: '26px', height: '26px', borderRadius: '50%', background: '#fff', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', boxShadow: '0 2px 6px rgba(0,0,0,0.2)' }}>
                    <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="#0D9488" strokeWidth="2.5" strokeLinecap="round"><path d="M11 4H4a2 2 0 00-2 2v14a2 2 0 002 2h14a2 2 0 002-2v-7"/><path d="M18.5 2.5a2.121 2.121 0 013 3L12 15l-4 1 1-4 9.5-9.5z"/></svg>
                  </div>
                )}
              </div>
            ...
            <div style={{ margin: '14px 14px 0', background: '#fff', borderRadius: '18px', padding: '16px', border: '1px solid #E2E8F0' }}>
              ...
              {[
                { label: 'Full Name',  val: name,  setter: setName,  editable: true },
                { label: 'Email',      val: email, setter: setEmail, editable: true },
                { label: 'Phone',      val: profile?.phone || '+91 98765 43210', setter: () => {}, editable: false },
                { label: 'City',       val: city,  setter: setCity,  editable: true },
                { label: 'Experience', val: exp,   setter: setExp,   editable: true },
              ].map((f, i) => (
                <div key={i} style={{ marginBottom: '12px' }}>
                  <label style={{ fontSize: '12px', fontWeight: 600, color: '#64748B', display: 'block', marginBottom: '5px' }}>{f.label}</label>
                  <input
                    value={f.val}
                    readOnly={!editMode || !f.editable}
                    onChange={e => f.setter(e.target.value)}
                    style={inputStyle(editMode && f.editable)}
                  />
                </div>
              ))}
            </div>
            ...
            <div style={{ margin: '12px 14px 0', background: '#fff', borderRadius: '18px', padding: '16px', border: '1px solid #E2E8F0' }}>
              ...
              <div style={{ display: 'flex', flexWrap: 'wrap', gap: '8px' }}>
                {SKILLS.map(s => (
                  <div key={s} style={{ background: '#EDFAF7', border: '1px solid #CCFBF1', borderRadius: '100px', padding: '6px 14px', fontSize: '12px', fontWeight: 700, color: '#0D9488' }}>{s}</div>
                ))}
              </div>
            </div>
            ...
            <div style={{ margin: '12px 14px 0', background: '#fff', borderRadius: '18px', padding: '16px', border: '1px solid #E2E8F0' }}>
              ...
              <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap' }}>
                {LANGS.map(l => (
                  <div key={l} style={{ background: '#EFF6FF', border: '1px solid #BFDBFE', borderRadius: '100px', padding: '6px 14px', fontSize: '12px', fontWeight: 700, color: '#2563EB' }}>{l}</div>
                ))}
              </div>
            </div>
              ...
              {DOCS.map(doc => {
                ...
                return (
                  <div key={doc.id} onClick={() => onDocPress(doc)}
                    ...
                    <div style={{ background: b.bg, color: b.color, fontSize: '11px', fontWeight: 700, padding: '4px 10px', borderRadius: '100px' }}>
                      {b.icon} {b.label}
                    </div>
                  </div>
                )
