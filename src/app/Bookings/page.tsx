...
const SEED_BOOKINGS: Booking[] = [
  { id:'b001', customerName:'Priya Sharma',  customerPhone:'+91 98765 11111', service:'Elder Care',           address:'B-42, Rohini Sector 11, Delhi',   date:'Today',       time:'3:00 PM',  duration:'2 hrs',   fare:480, status:'upcoming' },
  { id:'b002', customerName:'Amit Verma',    customerPhone:'+91 98765 22222', service:'Post-Surgery Care',    address:'A-12, Vasant Kunj, Delhi',        date:'Tomorrow',    time:'10:00 AM', duration:'3 hrs',   fare:720, status:'upcoming' },
  { id:'b003', customerName:'Sunita Gupta',  customerPhone:'+91 98765 33333', service:'Physiotherapy',        address:'22, Lajpat Nagar II, Delhi',      date:'28 Mar 2026', time:'11:00 AM', duration:'1.5 hrs', fare:360, status:'completed', rating:5 },
  { id:'b004', customerName:'Rajesh Mehta',  customerPhone:'+91 98765 44444', service:'Medication Assistance',address:'C-8, Dwarka Sector 6, Delhi',     date:'25 Mar 2026', time:'9:00 AM',  duration:'1 hr',    fare:240, status:'completed', rating:4 },
  { id:'b005', customerName:'Kavita Nair',   customerPhone:'+91 98765 55555', service:'Elder Care',           address:'5, Green Park, Delhi',            date:'20 Mar 2026', time:'2:00 PM',  duration:'2 hrs',   fare:480, status:'completed', rating:5 },
  { id:'b006', customerName:'Deepak Singh',  customerPhone:'+91 98765 66666', service:'Nursing',              address:'D-14, Pitampura, Delhi',          date:'18 Mar 2026', time:'4:00 PM',  duration:'2 hrs',   fare:560, status:'cancelled', cancelReason:'Customer cancelled' },
  { id:'b007', customerName:'Meena Joshi',   customerPhone:'+91 98765 77777', service:'First Aid',            address:'9, Saket, Delhi',                 date:'15 Mar 2026', time:'8:00 AM',  duration:'1 hr',    fare:280, status:'cancelled', cancelReason:'Partner unavailable' },
]
...
function BookingDetailSheet({ booking, onClose, onCancel }: { booking:Booking; onClose:()=>void; onCancel:(id:string)=>void }) {
  const SC = { upcoming:{bg:'#EDFAF7',color:'#0D9488',label:'Upcoming',icon:'🕐'}, completed:{bg:'#DCFCE7',color:'#16A34A',label:'Completed',icon:'✅'}, cancelled:{bg:'#FEE2E2',color:'#DC2626',label:'Cancelled',icon:'❌'} }
  const sc = SC[booking.status]
  return (
    <div onClick={e=>{if(e.target===e.currentTarget)onClose()}} style={{ position:'absolute', inset:0, zIndex:200, background:'rgba(15,23,42,0.55)', display:'flex', flexDirection:'column', justifyContent:'flex-end' }}>
      <div style={{ background:'#fff', borderRadius:'28px 28px 0 0', padding:'0 20px 36px', maxHeight:'88%', overflowY:'auto' }}>
        ...
        <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', marginBottom:'16px' }}>
          ...
          <div style={{ background:sc.bg, color:sc.color, fontSize:'12px', fontWeight:700, padding:'5px 12px', borderRadius:'100px' }}>{sc.icon} {sc.label}</div>
        </div>
        ...
        <div style={{ background:'#fff', borderRadius:'16px', border:'1px solid #E2E8F0', padding:'14px 16px', marginBottom:'12px' }}>
          ...
          <div style={{ display:'flex', alignItems:'center', gap:'12px' }}>
            <div style={{ width:'44px', height:'44px', borderRadius:'50%', background:'linear-gradient(135deg,#0D9488,#065f52)', display:'flex', alignItems:'center', justifyContent:'center', fontSize:'18px', color:'#fff', fontWeight:800 }}>{booking.customerName.charAt(0)}</div>
            <div>
        ...
        <div style={{ background:'#fff', borderRadius:'16px', border:'1px solid #E2E8F0', padding:'14px 16px', marginBottom:'12px' }}>
          ...
          {[{icon:'🏥',label:'Service',val:booking.service},{icon:'📅',label:'Date',val:booking.date},{icon:'🕐',label:'Time',val:booking.time},{icon:'⏱️',label:'Duration',val:booking.duration},{icon:'📍',label:'Address',val:booking.address}].map((r,i,arr) => (
        ...
        {booking.status==='completed'&&booking.rating&&(
          <div style={{ background:'#FEF3C7', borderRadius:'14px', padding:'12px 16px', marginBottom:'12px', display:'flex', alignItems:'center', gap:'10px', border:'1px solid #FDE68A' }}>
            ...
            <div>
              ...
              <div style={{ display:'flex', gap:'3px', marginTop:'4px' }}>{[1,2,3,4,5].map(s=><span key={s} style={{ fontSize:'16px' }}>{s<=booking.rating!?'★':'☆'}</span>)}</div>
            </div>
          </div>
        )}
        ...
        {booking.status==='upcoming'&&(
          <button onClick={()=>{onCancel(booking.id);onClose()}} style={{ width:'100%', padding:'15px', background:'#FEE2E2', border:'1.5px solid #FECACA', borderRadius:'14px', color:'#DC2626', fontSize:'15px', fontWeight:700, cursor:'pointer', fontFamily:'DM Sans, sans-serif', marginBottom:'10px' }}>Cancel Booking</button>
        )}
        <button onClick={onClose} style={{ width:'100%', padding:'15px', background:'transparent', border:'1.5px solid #E2E8F0', borderRadius:'14px', color:'#64748B', fontSize:'15px', fontWeight:700, cursor:'pointer', fontFamily:'DM Sans, sans-serif' }}>Close</button>
      </div>
    </div>
  )
}
...
function BookingCard({ booking, onPress }: { booking:Booking; onPress:()=>void }) {
  const SC = { upcoming:{bg:'#EDFAF7',color:'#0D9488',label:'Upcoming'}, completed:{bg:'#DCFCE7',color:'#16A34A',label:'Completed'}, cancelled:{bg:'#FEE2E2',color:'#DC2626',label:'Cancelled'} }
  const sc = SC[booking.status]
  return (
    <div onClick={onPress} style={{ margin:'0 14px 10px', background:'#fff', borderRadius:'18px', padding:'14px 16px', border:'1px solid #E2E8F0', cursor:'pointer' }}>
      <div style={{ display:'flex', alignItems:'flex-start', justifyContent:'space-between', marginBottom:'10px' }}>
        <div style={{ flex:1, minWidth:0, marginRight:'10px' }}>
          <div style={{ fontSize:'15px', fontWeight:800, color:'#0F172A' }}>{booking.customerName}</div>
          <div style={{ fontSize:'12px', color:'#64748B', marginTop:'2px' }}>{booking.service}</div>
        </div>
        <div style={{ background:sc.bg, color:sc.color, fontSize:'10px', fontWeight:700, padding:'4px 10px', borderRadius:'100px', flexShrink:0 }}>{sc.label}</div>
      </div>
      <div style={{ display:'flex', gap:'6px', flexWrap:'wrap', marginBottom:'12px' }}>
        {[{icon:'📅',val:booking.date},{icon:'🕐',val:booking.time},{icon:'⏱️',val:booking.duration}].map(item=>(
      ...
      <div style={{ display:'flex', alignItems:'center', gap:'6px', marginBottom:'12px' }}>
        <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="#94A3B8" strokeWidth="2.5" strokeLinecap="round"><path d="M21 10c0 7-9 13-9 13s-9-6-9-13a9 9 0 0118 0z"/><circle cx="12" cy="10" r="3"/></svg>
...
export default function BookingsPage() {
  ...
  const handleCancel = (id: string) => {
    showPopup({
      type:'confirm', title:'Cancel Booking?', body:'Are you sure you want to cancel this booking?\nThis action cannot be undone.', icon:'⚠️',
      actions:[
        { label:'Yes, Cancel', variant:'danger', fn:() => {
          setBookings(prev => prev.map(b => b.id===id ? {...b, status:'cancelled' as BookingStatus, cancelReason:'Cancelled by partner'} : b))
          closePopup()
    ...
    <MobileFrame>
      <div style={{ position:'absolute', inset:0, display:'flex', flexDirection:'column' }}>
        ...
        <div style={{ background:'#fff', padding:'10px 14px', display:'flex', gap:'8px', flexShrink:0, borderBottom:'1px solid #E2E8F0' }}>
          {TABS.map(t => (
            <button key={t.key} onClick={() => setActiveTab(t.key)} style={{ flex:1, padding:'8px 4px', borderRadius:'12px', border:'none', cursor:'pointer', background:activeTab===t.key?'#0D9488':'#F1F5F9', fontFamily:'DM Sans, sans-serif', transition:'all 0.2s' }}>
              <div style={{ fontSize:'14px' }}>{t.icon}</div>
              <div style={{ fontSize:'10px', fontWeight:700, color:activeTab===t.key?'#fff':'#64748B', marginTop:'2px' }}>{t.label}</div>
              {TAB_COUNT(t.key)>0&&<div style={{ fontSize:'10px', fontWeight:800, color:activeTab===t.key?'rgba(255,255,255,0.8)':'#94A3B8' }}>({TAB_COUNT(t.key)})</div>}
            </button>
          ))}
        </div>
      ...
      <PopupLayer />
      {selectedBooking && <BookingDetailSheet booking={selectedBooking} onClose={() => setSelectedBooking(null)} onCancel={handleCancel} />}
    </MobileFrame>
  )
}