...
export default function DocumentsPage() {
  ...
  const handleUpload = () => {
    showPopup({
      type: 'info', title: 'Upload Document',
      body: 'Choose a document type to upload.\nSupported: PDF, JPG, PNG (max 10MB)',
      icon: '📤',
      actions: [
  ...
  const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file.size > 10 * 1024 * 1024) {
      showPopup({ type: 'error', title: 'File Too Large', body: 'Please select a file smaller than 10MB.', icon: '❌', actions: [{ label: 'OK', variant: 'primary', fn: closePopup }] })
      return
    }
    setUploading(true)
    setTimeout(() => {
  ...
  const handleDelete = (doc: Doc) => {
    showPopup({
      type: 'confirm', title: 'Delete Document?',
        ...
        <div style={{ position: 'absolute', inset: 0, display: 'flex', flexDirection: 'column' }}>
          ...
          <div style={{ background: '#fff', padding: '10px 14px', display: 'flex', gap: '8px', overflowX: 'auto', flexShrink: 0, borderBottom: '1px solid #E2E8F0' }}>
            {CATEGORIES.map(cat => (
              <button key={cat} onClick={() => setActiveTab(cat)} style={{
                padding: '6px 14px', borderRadius: '100px', border: 'none', cursor: 'pointer', flexShrink: 0,
                background: activeTab === cat ? '#0D9488' : '#F1F5F9',
          ...
          <div style={{ flex: 1, overflowY: 'auto', padding: '12px 14px', paddingBottom: '90px' }}>
            {filtered.length === 0 ? (
              ...
              <div key={doc.id} style={{ background: '#fff', borderRadius: '16px', padding: '14px', marginBottom: '10px', display: 'flex', alignItems: 'center', gap: '12px', border: '1px solid #E2E8F0' }}>
                <div style={{ width: '46px', height: '46px', borderRadius: '13px', background: '#F1F5F9', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '22px', flexShrink: 0 }}>
                  {doc.icon}
                </div>
                ...
                <button onClick={() => handleDelete(doc)} style={{ width: '32px', height: '32px', borderRadius: '50%', background: '#FEE2E2', border: 'none', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', flexShrink: 0 }}>
