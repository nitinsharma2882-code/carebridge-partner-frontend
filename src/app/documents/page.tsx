...
export default function DocumentsPage() {
  ...
  const handleView = (doc: Doc) => showPopup({
    type:'info', title:`Viewing: ${doc.name}`, icon:'👁️',
    body:`Opening ${doc.name} for preview.\nFile size: ${doc.size}\nUploaded: ${doc.date}`,
    actions:[{ label:'Close', variant:'secondary', fn:closePopup }],
  })

  const handleDownload = (doc: Doc) => showPopup({
    type:'success', title:'Download Started', icon:'⬇️',
    body:`${doc.name} is being downloaded.\nFile size: ${doc.size}`,
    ...
    if (navigator.share) {
      try {
        await navigator.share({
          title: doc.name,
          text: `Sharing document: ${doc.name} (${doc.size}) — uploaded on ${doc.date}`,
      ...
      try {
        await navigator.clipboard.writeText(`${doc.name} — ${window.location.href}`)
        showPopup({
          type:'success', title:'Link Copied! 📋', icon:'📋',
          body:'Document link has been copied to your clipboard.',
          actions:[{ label:'OK', variant:'primary', fn:closePopup }],
        })
      } catch {
        showPopup({
          type:'info', title:'Share Document', icon:'📤',
          body:`Share "${doc.name}" via WhatsApp, Email, or any app.`,
          actions:[{ label:'OK', variant:'primary', fn:closePopup }],
        })
      }
    }
            ...
            <div key={doc.id} style={{ background:'#fff', borderRadius:'16px', padding:'14px', marginBottom:'10px', border:'1px solid #E2E8F0' }}>
              ...
              <div style={{ display:'flex', gap:'6px' }}>
                <button onClick={() => handleView(doc)}
                  style={{ flex:1, padding:'8px 4px', background:'#F8FAFC', border:'1px solid #E2E8F0', borderRadius:'10px', fontSize:'11px', fontWeight:700, color:'#475569', cursor:'pointer', fontFamily:'DM Sans, sans-serif', display:'flex', alignItems:'center', justifyContent:'center', gap:'4px' }}>
                ...
                <button onClick={() => handleDownload(doc)}
                  style={{ flex:1, padding:'8px 4px', background:'#EDFAF7', border:'1px solid #CCFBF1', borderRadius:'10px', fontSize:'11px', fontWeight:700, color:'#0D9488', cursor:'pointer', fontFamily:'DM Sans, sans-serif', display:'flex', alignItems:'center', justifyContent:'center', gap:'4px' }}>
                ...
                <button onClick={() => handleShare(doc)}
                  style={{ flex:1, padding:'8px 4px', background:'#EFF6FF', border:'1px solid #BFDBFE', borderRadius:'10px', fontSize:'11px', fontWeight:700, color:'#2563EB', cursor:'pointer', fontFamily:'DM Sans, sans-serif', display:'flex', alignItems:'center', justifyContent:'center', gap:'4px' }}>
                ...
                <button onClick={() => handleDelete(doc)}
                  style={{ width:'36px', padding:'8px', background:'#FEF2F2', border:'1px solid #FECACA', borderRadius:'10px', cursor:'pointer', display:'flex', alignItems:'center', justifyContent:'center', flexShrink:0 }}>
