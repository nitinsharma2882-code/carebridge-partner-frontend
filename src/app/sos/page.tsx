...
export default function SOSPage() {
  ...
  const [seconds,   setSeconds]   = useState(0)   // seconds since connecting
  ...
  // ── Connecting phase — reveal steps progressively ──
  const startConnecting = () => {
    ...
    // Step 3 — after 3.5s → show connected
    setTimeout(() => {
  ...
  const updateStep = (id: string, status: JourneyStep['status'], label?: string, sub?: string) => {
    setSteps(prev => prev.map(s =>
      s.id === id ? { ...s, status, ...(label ? { label } : {}), ...(sub !== undefined ? { sub } : {}) } : s
    ))
  }
  ...
  return (
    <div style={{ minHeight:'100vh', background:'#0a0f1e', display:'flex', alignItems:'center', justifyContent:'center' }}>
      <div style={{ width:'390px', height:'844px', borderRadius:'48px', overflow:'hidden', position:'relative', flexShrink:0, boxShadow:'0 0 0 10px #1e293b, 0 0 0 12px #334155, 0 40px 80px rgba(0,0,0,0.8)' }}>
        ...
        {/* Full screen red bg */}
        <div style={{ position:'absolute', inset:0, background:'linear-gradient(160deg,#7f1d1d,#DC2626 55%,#ef4444)', display:'flex', flexDirection:'column', overflowY:'auto' }}>
