    ...
    <MobileFrame>
      <div style={{ position:'absolute', inset:0, display:'flex', flexDirection:'column' }}>
        ...
        <div style={{ height:'56px', background:settings.highContrast?'#111':'#fff', display:'flex', alignItems:'center', padding:'0 16px', gap:'10px', borderBottom:`1px solid ${settings.highContrast?'#333':'#E2E8F0'}`, flexShrink:0 }}>
          ...
          {hasChanges
            ? <button onClick={save} style={{ background:'#0D9488', border:'none', borderRadius:'10px', padding:'0 14px', height:'34px', fontSize:'13px', fontWeight:700, color:'#fff', cursor:'pointer', fontFamily:'DM Sans, sans-serif' }}>Save</button>
        ...
        <div style={{ flex:1, overflowY:'auto', padding:'0 14px 32px', background:settings.highContrast?'#000':'#F8FAFC' }}>
          ...
          <div style={{ background:'#fff', borderRadius:'18px', padding:'16px', border:'1px solid #E2E8F0' }}>
            ...
            <div style={{ display:'flex', gap:'8px' }}>
              {FONT_OPTIONS.map(f => (
                <button key={f.key} onClick={()=>update('fontSize', f.key)} style={{ flex:1, padding:'10px 4px', borderRadius:'12px', border:'none', cursor:'pointer', background:settings.fontSize===f.key?'#0D9488':'#F1F5F9', fontFamily:'DM Sans, sans-serif', transition:'all 0.15s', display:'flex', flexDirection:'column', alignItems:'center', gap:'4px' }}>
